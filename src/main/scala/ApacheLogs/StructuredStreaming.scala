...
object StructuredStreaming {
  ...
  Logger.getLogger("akka").setLevel(Level.ERROR)
   ...
   // Function to convert Apache log times to what Spark/SQL expects using java functions
   ...
   def main(args: Array[String]) {
      ...
      val spark = SparkSession
        ...
        .master("local[*]")
        .config("spark.sql.streaming.checkpointLocation", "file:////Users/franziskaadler/spark_streaming/checkpoint")
        .getOrCreate()
        
      setupLogging()
        
      // Create a stream of text files dumped into the logs directory
      // rawData is continiously growing dataframe
      // searches in this folder ../logs for txt files to load
      val rawData = spark.readStream.text("/Users/franziskaadler/spark_streaming/logs")
