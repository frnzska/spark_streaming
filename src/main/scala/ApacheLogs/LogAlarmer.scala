...
/** Monitors a stream of Apache access logs on port 9998, and prints an alarm
...
object LogAlarmer {
  ...
  def main(args: Array[String]) {
    ...
    val statuses = lines.map(x => {
          ...
          if (matcher.matches()) matcher.group(6) else "[error]" // field 6 is the status code
        }
    )
    
    // Now map these status results to success and failure
    val successFailure = statuses.map(x => {
      val statusCode = util.Try(x.toInt) getOrElse 0 //try to convert status code to int else zero
